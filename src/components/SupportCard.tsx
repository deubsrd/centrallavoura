...
const SupportCard = ({ title, description, links, icon, index }: SupportCardProps) => {
  return (
    ...
    >
      ...
      <h3 className="mt-4 font-display text-xl font-semibold text-foreground tracking-wide">{title}</h3>
      <p className="mt-1.5 text-sm font-body text-muted-foreground leading-relaxed">{description}</p>
      <div className="mt-4 flex flex-wrap gap-2">
        {links.map((link, i) => {
          const isWA = link.isWhatsApp !== false && link.href.includes("wa.me");
          return (
            <a
              key={i}
              href={link.href}
              ...
              className={`inline-flex items-center gap-1.5 rounded-md px-4 py-2 text-sm font-medium font-body transition-colors ${
                isWA
            ...
            >
              {isWA && <WhatsAppIcon />}
              {link.label}
              {!isWA && <ExternalLink className="h-3.5 w-3.5" />}
            </a>
          );
        })}
      </div>
